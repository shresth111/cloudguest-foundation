    ...
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      {breadcrumb && (
        <p className="text-xs text-slate-400">
          YOU ARE HERE : <span className="text-primary">Wyfy Guest</span> / {breadcrumb}
        </p>
      )}
    </div>
  );
  ...
  return (
    <span className="relative inline-flex">
      <button
        ...
        className="inline-flex items-center justify-center rounded text-slate-300 hover:text-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-500"
      >
...
const STATUS_COLORS: Record<string, string> = {
  active:
    "bg-emerald-100 text-emerald-700 ring-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-400",
...
export function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-block rounded-full px-2.5 py-0.5 text-[11px] font-medium ring-1 ${cls}`}
    >
      {status}
    </span>
  );
    ...
    <div>
        ...
        {onSearch && (
          <div className="relative">
            ...
            <input
              ...
              className="w-44 rounded-md border border-slate-200 py-1.5 pl-8 pr-3 text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500"
            />
          </div>
        )}
        <div className="flex items-center gap-0.5 rounded-md border border-slate-200 p-0.5">
          {[10, 25, 50].map((n) => (
            <button
              ...
              className={`rounded px-2 py-1 text-xs font-medium ${pageSize === n ? "bg-slate-900 text-white" : "text-slate-500 hover:bg-slate-100"}`}
            >
              {n}
            </button>
          ))}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[800px] w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 text-left">
              {cols.map((c) => (
                <th
                  key={c.key}
                  className={`pb-2 pr-3 ${setSortKey ? "cursor-pointer select-none" : ""}`}
          ...
          <tbody>
            {rows}
            {total === 0 && <EmptyState title="No data to show." />}
          </tbody>
        </table>
      </div>
      {total > 0 && (
        <div className="mt-4 flex items-center justify-between text-xs text-slate-500">
          <span>
            Showing {sp * pageSize + 1}–{Math.min((sp + 1) * pageSize, total)} of {total}
          </span>
          <div className="flex items-center gap-1">
            <button
              disabled={sp === 0}
              onClick={() => setPage(sp - 1)}
              className="inline-flex items-center justify-center rounded p-1 disabled:opacity-40 focus:outline-none focus:ring-2 focus:ring-orange-500"
            >
    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          max={max}
          className="block w-full rounded-md border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 dark:bg-slate-700 dark:text-slate-100"
        />
...
export function InlineConfirm({ onConfirm }: { onConfirm: () => void }) {
  ...
  return (
    <button
      onClick={handleClick}
      className={`inline-flex items-center justify-center rounded p-1 transition-colors ${arming ? "bg-orange-500 text-white" : "text-slate-400 hover:text-red-500"}`}
    >
      <span className="text-[11px] font-medium px-1">
        {arming ? "Confirm" : <Trash2 className="h-4 w-4" />}
      </span>
    </button>
  );
}
