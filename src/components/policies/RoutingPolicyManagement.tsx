...
import { Plus, Search, Trash2, Pencil, Network, Activity, ListChecks } from "lucide-react";
...
export function RoutingPolicyManagement() {
  const { data: policies = [], isLoading } = useRoutingPolicies();
  const { data: kpis } = useRoutingPolicyKpis();
  const save = useSaveRoutingPolicy();
  const del = useDeleteRoutingPolicy();
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        description="Freeform routing rules -- no fixed schema on the backend, so rules are edited as raw JSON. VLAN membership and firewall rules have their own dedicated pages under Network."
        actions={
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Total" value={kpis?.total ?? 0} icon={Network} tone="primary" />
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o) { onClose(); form.reset(); } }}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Rules are stored as raw JSON -- this policy type has no typed schema on the backend.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={form.handleSubmit(async (v) => { await onSave(v); form.reset(); })} className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            {policy && (
              <p className="text-[11px] text-muted-foreground sm:col-span-2">
                ...
                backend) -- only the rules JSON and status below can be changed.
              </p>
            )}
            <Field label="Status">
          ...
          <Field label="Rules (JSON)" error={form.formState.errors.rulesJson?.message}>
            <Textarea rows={8} className="font-mono text-xs" {...form.register("rulesJson")} />
          </Field>
