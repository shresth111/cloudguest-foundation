            ...
            <div className="flex items-center justify-between">
              <Switch
                ...
                onCheckedChange={(v) => {
                  setItems(
                    items.map((x) =>
                      x.id === p.id ? { ...x, enabled: v, status: v ? "active" : "draft" } : x,
                    ),
                  );
...
export function WhitelistPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold">Device Whitelist</h2>
        </div>
        <Button
          size="sm"
          onClick={() => {
            ...
            <TableBody>
              {items.map((v) => (
                <TableRow key={v.id} className="border-b">
                  <TableCell className="font-mono text-xs">{v.mac}</TableCell>
...
export function TeamsPage() {
  const [items, setItems] = useState<Team[]>([
    { id: "1", name: "Sales Team", members: 12, quota: 85, status: "active" },
    { id: "2", name: "Executive VIP", members: 5, quota: 42, status: "active" },
    { id: "3", name: "Contractors", members: 8, quota: 100, status: "active" },
  ]);
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold">Guest Teams</h2>
        </div>
        <Button
          size="sm"
          onClick={() => {
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((t) => (
          <Card key={t.id} className="shadow-sm border-0">
            <CardContent className="p-5">
              ...
              <div className="mt-1 h-1.5 rounded-full bg-muted overflow-hidden">
                <div className="h-full rounded-full bg-primary" style={{ width: `${t.quota}%` }} />
              </div>
              <div className="flex gap-2 mt-3">
                <Button
                  ...
                  onClick={() => toast.success(`${t.name} updated`)}
                >
                  Manage
                </Button>
                <Button
...
export function NetworkingPage() {
  const items: NetworkItem[] = [
    { id: "1", name: "VLAN", status: "active", detail: "5 VLANs configured" },
    { id: "2", name: "DHCP", status: "active", detail: "3 pools, 245 active leases" },
    { id: "3", name: "DNS", status: "active", detail: "8 records, 2 zones" },
    { id: "4", name: "Firewall", status: "active", detail: "12 rules enabled" },
    { id: "5", name: "Hotspot", status: "active", detail: "Enabled on 2 SSIDs" },
    { id: "6", name: "ISP", status: "warning", detail: "Primary + Backup configured" },
  ];
  const [rebooting, setRebooting] = useState(false);
  const reboot = () => {
    setRebooting(true);
    toast.promise(new Promise((res) => setTimeout(res, 2000)), {
      loading: "Rebooting network…",
      success: "Network rebooted",
      error: "Reboot failed",
    });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-card p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-5">
          ...
          <span className="flex items-center gap-2 text-sm">
            ...
            <span className="font-semibold">11-Nov-2026</span>
          </span>
        </div>
        <Button size="sm" variant="outline" disabled={rebooting} onClick={reboot}>
          <RotateCcw className={`h-4 w-4 ${rebooting ? "animate-spin" : ""}`} />
          Reboot
        </Button>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <Card key={item.id} className="shadow-sm border-0 hover:shadow-md cursor-pointer">
            <CardContent className="p-5">
              <div className="flex items-center justify-between mb-2">
              ...
              <Button
                ...
                onClick={() => toast.success(`${item.name} settings opened`)}
              >
                Configure
              </Button>
            </CardContent>
