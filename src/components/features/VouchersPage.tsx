...
const clamp = (n: number, fallback: number, lo: number, hi: number) =>
  Math.min(hi, Math.max(lo, Number.isNaN(n) ? fallback : n));
  ...
  return (
    ...
    >
      <g transform="translate(4, 8)">
        <path
          d="M0 4a4 4 0 0 1 4-4h30a4 4 0 0 1 4 4v18a4 4 0 0 1-4 4H4a4 4 0 0 1-4-4z"
          fill="#2e2a5c"
          stroke="#a78bfa"
          strokeWidth="1.5"
        />
      ...
      <motion.g
        animate={
          shouldReduceMotion ? { opacity: 0.9 } : { scale: [1, 1.06, 1], opacity: [0.85, 1, 0.85] }
        }
        transition={
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const batch = await voucherService.createBatch({
        name: form.name || `Batch ${Date.now()}`,
        organizationId: orgId,
        locationId,
        quantity: Math.min(form.quantity, 100),
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold tracking-tight">Voucher Batches</h2>
            <p className="text-sm text-muted-foreground">
              {rows.length} {demo ? "vouchers" : "batches"}
            </p>
          </div>
          <VoucherIssueIllustration />
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => toast.success("Bulk import started")}>
            Import CSV
          </Button>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              ...
              <div className="grid grid-cols-2 gap-3 py-2">
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={form.quantity}
                    onChange={(e) =>
                      setForm({ ...form, quantity: clamp(parseInt(e.target.value), 1, 1, 100) })
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    value={form.validMin}
                    onChange={(e) =>
                      setForm({ ...form, validMin: clamp(parseInt(e.target.value), 60, 1, 43200) })
                    }
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={form.codeLen}
                    onChange={(e) =>
                      setForm({ ...form, codeLen: clamp(parseInt(e.target.value), 8, 4, 16) })
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, dataLimit: clamp(parseInt(e.target.value), 0, 0, 102400) })
                    }
                  />
                </div>
                <div>
              ...
              <DialogFooter>
                ...
                <Button onClick={handleGenerate}>Generate {form.quantity} Vouchers</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <EmptyState
              ...
              description="Generate a batch above to issue guest access vouchers."
            />
          ) : (
                ...
                <TableHeader>
                  <TableRow>
                    <TableHead className="text-xs font-medium">
                      {demo ? "Code" : "Batch Name"}
                    </TableHead>
                    {demo && <TableHead className="text-xs font-medium">Business Unit</TableHead>}
                    <TableHead className="text-xs font-medium">
                      {demo ? "Plan" : "Validity"}
                    </TableHead>
                    ...
                    {demo && <TableHead className="text-xs font-medium">Redeemed At</TableHead>}
                      ...
                      {demo ? (
                        <TableCell>
                          <Select
                            ...
                            onValueChange={(val) => {
                              setItems((its) =>
                                its.map((it) => (it.code === v.code ? { ...it, plan: val } : it)),
                              );
                              toast.success(`Plan updated to ${formatPlan(val)}`);
                            }}
                          >
      ...
      {demo && (
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => toast.success("Print job queued")}>
            <Printer className="mr-1.5 h-3.5 w-3.5" />
            Print
          </Button>
          <Button variant="outline" size="sm" onClick={() => toast.success("Email sent")}>
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {demo ? "Voucher" : "Batch"} — {viewBatch?.code}
            </DialogTitle>
          </DialogHeader>
          {demo && viewBatch ? (
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span>{formatPlan(viewBatch.plan)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Status</span>
                <Badge
                  variant={viewBatch.status === "active" ? "default" : "secondary"}
                  className="capitalize"
                >
                  {viewBatch.status}
                </Badge>
              ...
              <div className="flex justify-between">
                ...
                <span>{viewBatch.used}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Business Unit</span>
                <span>{viewBatch.businessUnit}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Redeemed At</span>
                <span>{viewBatch.redeemedAt ?? "—"}</span>
              </div>
            </div>
          ) : viewLoading ? (
              ...
              {viewStats && (
                <div className="grid grid-cols-3 gap-2 text-center text-xs">
                  <div className="rounded-lg border p-2">
                    <p className="text-lg font-semibold">{viewStats.total}</p>
                    <p className="text-muted-foreground">Total</p>
                  </div>
                  <div className="rounded-lg border p-2">
                    <p className="text-lg font-semibold">{viewStats.unused}</p>
                    <p className="text-muted-foreground">Unused</p>
                  </div>
                  <div className="rounded-lg border p-2">
                    <p className="text-lg font-semibold">
                      {(viewStats.redemptionRate * 100).toFixed(0)}%
                    </p>
                    ...
                    {viewVouchers.length === 0 ? (
                      ...
                      viewVouchers.map((vch) => (
                        <TableRow key={vch.id}>
                          ...
                          <TableCell>
                            <Badge
                              variant={vch.status === "unused" ? "secondary" : "default"}
                              className="capitalize text-[10px]"
                            >
                              {vch.status.replace(/_/g, " ")}
                            </Badge>
                          </TableCell>
              ...
              <div className="space-y-2 rounded-lg border p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={() => exportBatchCsv(viewBatch as BatchRow)}
                  >
                    <Download className="mr-1.5 h-3.5 w-3.5" />
                  ...
                  <Button
                    ...
                    className="h-8 text-xs"
                    disabled={exporting !== null}
                    onClick={() => downloadBatchPdf(viewBatch as BatchRow)}
                  >
                    <Printer className="mr-1.5 h-3.5 w-3.5" />
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="h-8 flex-1 rounded-md border border-input bg-background px-2.5 text-xs outline-none focus:border-primary focus:ring-2 focus:ring-primary/15"
                  />
                  <Button
                    size="sm"
                    className="h-8 text-xs"
                    disabled={emailing || !emailTo.trim()}
                    onClick={() => emailBatch(viewBatch as BatchRow)}
                  >
                    <Mail className="mr-1.5 h-3.5 w-3.5" />
