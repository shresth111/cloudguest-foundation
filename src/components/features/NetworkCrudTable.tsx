...
export const validators = {
  ...
  requiredPort: (v: string) =>
    v && PORT_RE.test(v) && +v > 0 && +v <= 65535 ? null : "Please enter a valid port",
  required: (label: string) => (v: string) =>
    v ? null : `Please enter a valid ${label.toLowerCase()}`,
};
...
export default function NetworkCrudTable({
  title,
  description,
  caution,
  tableTitle,
  ...
  const remove = (id: string) => {
    setRows((p) => p.filter((r) => r.__id !== id));
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={cn(inputCls, "w-44 py-1.5 pl-8")}
              />
            </div>
            <button
              onClick={openAdd}
                ...
                paged.map((r) => (
                  <tr key={r.__id} className="border-b last:border-0 hover:bg-accent/50">
                    {columns.map((c) => {
                      ...
                      return (
                        <td key={c.key} className="px-3 py-2.5 text-xs text-foreground">
              ...
              {fields.map((f) => (
                <div key={f.key}>
                  <label className={labelCls}>
                    {f.label}
                    {f.validate && <span className="text-destructive"> *</span>}
                  </label>
                  {f.type === "select" ? (
                    <select
