    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        rows.push({
          ...
          status,
          operatorResponse:
            OPERATOR_RESPONSES[Math.floor(Math.random() * OPERATOR_RESPONSES.length)],
        });
      }
      resolve(rows);
    }, 700);
...
export default function OtpDeliveryReport({ onNavigate }: Props) {
  ...
  const handleSearch = useCallback(async () => {
    ...
    setResults(data);
    setSearchedFor(
      `${cleaned} on ${pad2(parseInt(date.slice(8, 10)))}-${pad2(parseInt(date.slice(5, 7)))}-${date.slice(0, 4)}`,
    );
    setSearching(false);
  }, [mobile, date]);
  ...
  const statusPill = (s: OtpRow["status"]) => {
    ...
    return (
      <span
        className={`inline-block rounded-full px-2.5 py-0.5 text-[11px] font-medium ${m[s] ?? m.Expired}`}
      >
        {s}
      </span>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg bg-white p-6 ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-slate-600">
        <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">
          Search OTP Delivery Report
        </h2>
        <p className="mt-1 text-sm text-slate-400 dark:text-slate-500">
          While logging into the Hotspot, OTPs are sometimes not delivered or arrive late because of
          the telecom operator. Track them here.
        </p>
              ...
              <table className="min-w-[900px] w-full text-sm">
                ...
                <tbody>
                  {results.map((r, i) => (
                    <tr
                      key={r.id}
                      className="border-b border-slate-100 text-slate-700 last:border-0 dark:border-slate-700 dark:text-slate-300"
                    >
                      ...
                      <td className="py-2.5 pr-3 text-xs">
                        <span className={r.attempts > 1 ? "font-semibold text-orange-600" : ""}>
                          {r.attempts}
                        </span>
                      </td>
                      <td className="py-2.5 pr-3">{statusPill(r.status)}</td>
                      <td
                        className="max-w-[140px] truncate py-2.5 text-xs text-slate-400 dark:text-slate-500"
                        title={r.operatorResponse}
                      >
                        {r.operatorResponse}
                      </td>
                    </tr>
