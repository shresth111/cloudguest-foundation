...
import {
  ...
  ChevronLeft, ChevronRight, Trash2, RotateCcw, Undo2, Ban, Smartphone, Mail,
} from "lucide-react";
...
export default function BlockUsers({ locationId }: { locationId?: string } = {}) {
  ...
  const [blocked, setBlocked] = useState<BlockedUser[]>(demo ? [
    { id: "b1", name: "Ravi Sharma", identifier: "+919876543210", businessUnit: "Mumbai HQ", blockedOn: "2026-07-20T10:00:00.000Z", status: "Blocked" },
    { id: "b2", name: null, identifier: "+919812345678", businessUnit: "Delhi Office", blockedOn: "2026-07-18T10:00:00.000Z", status: "Blocked" },
    { id: "b3", name: "Priya Kapoor", identifier: "priya.kapoor@example.com", businessUnit: "Mumbai HQ", blockedOn: "2026-07-22T10:00:00.000Z", status: "Blocked" },
    { id: "b4", name: "Amit Patel", identifier: "+919722233344", businessUnit: "Bangalore DC", blockedOn: "2026-07-13T10:00:00.000Z", status: "Unblocked" },
    { id: "b5", name: "Sana Khan", identifier: "sana.khan@example.com", businessUnit: "Chennai Office", blockedOn: "2026-07-21T10:00:00.000Z", status: "Blocked" },
    { id: "b6", name: "John Doe", identifier: "+919655566677", businessUnit: "Delhi Office", blockedOn: "2026-07-16T10:00:00.000Z", status: "Blocked" },
  ] : []);
  ...
  const identifierNoun = (n: number) =>
    mode === "email" ? (n === 1 ? "email address" : "email addresses") : (n === 1 ? "number" : "numbers");
  ...
  const parsed = useMemo(() => {
    ...
    const blockedIdentifiers = new Set(
      blocked.filter((b) => b.status === "Blocked").map((b) =>
        mode === "mobile" ? b.identifier.replace(/[\s\-\+]/g, "") : b.identifier,
      ),
    );
    ...
    for (const c of cleaned) {
      ...
      seen.add(c);
      if (blockedIdentifiers.has(c)) { alreadyBlocked.push(c); continue; }
      if (mode === "mobile") {
  ...
  const chipNumbers = useMemo(() => {
    ...
    for (let i = 0; i < cleaned.length; i++) {
      ...
      seen.add(cleaned[i]);
      if (blockedIdentifiers.has(cleaned[i])) { result.push({ num: mode === "mobile" ? "+" + cleaned[i] : cleaned[i], status: "blocked", raw: raw[i] }); continue; }
      if (mode === "mobile") {
        ...
        if (EMAIL_RE.test(cleaned[i])) { result.push({ num: cleaned[i], status: "valid", raw: raw[i] }); }
        else { result.push({ num: raw[i], status: "invalid", raw: raw[i] }); }
      }
    }
    return result;
  ...
  const filtered = useMemo(() => {
    ...
    let items = blocked.filter((b) => (!demo || b.businessUnit === bu) && (!q || b.name?.toLowerCase().includes(q) || b.identifier.toLowerCase().includes(q) || b.businessUnit.toLowerCase().includes(q) || b.status.toLowerCase().includes(q)));
    items.sort((a, b) => {
  ...
  const handleBlock = async () => {
    ...
    if (demo) {
      const newBlocked = parsed.valid.map((m) => ({ id: `b${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, name: null, identifier: m, businessUnit: bu, blockedOn: now, status: "Blocked" as const }));
  ...
  const handlePrimaryClick = () => {
    ...
    if (parsed.alreadyBlocked.length > 0 && parsed.invalid.length === 0) {
      msg = parsed.alreadyBlocked.length === 1 ? `That ${identifierNoun(1)} is already blocked.` : `Those ${identifierNoun(2)} are already blocked.`;
  ...
  const toggleStatus = async (id: string) => {
    ...
    try {
      if (row.status === "Blocked") {
        ...
        const created = await guestService.createAccessRule({ kind: "identifier", organizationId: orgId, locationId, identifier: row.identifier, ruleType: "blocklist" });
  ...
  return (
    <div className="space-y-6">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => { setShowModal(false); triggerRef.current?.focus(); }}>
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl dark:bg-slate-800" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="block-modal-title">
            <h3 id="block-modal-title" className="text-lg font-semibold text-slate-800 dark:text-slate-100" tabIndex={-1}>Block {parsed.valid.length} {identifierNoun(parsed.valid.length)}?</h3>
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button onClick={handleBlock} className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Block {mode === "email" ? "emails" : "numbers"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Cut off a guest's access to your network immediately.</p>
          </div>
        </div>
      ...
      <CardContent>
        ...
        <div role="tablist" aria-label="Block by" className="mb-4 inline-flex items-center gap-0.5 rounded-lg bg-slate-100 p-0.5 dark:bg-slate-700">
          <button type="button" role="tab" aria-selected={mode === "mobile"} onClick={() => handleModeChange("mobile")} className={cn("inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-colors", mode === "mobile" ? "bg-white text-slate-800 shadow-sm dark:bg-slate-600 dark:text-slate-100" : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200")}>
            <Smartphone className="h-3.5 w-3.5" /> Mobile number
          </button>
          <button type="button" role="tab" aria-selected={mode === "email"} onClick={() => handleModeChange("email")} className={cn("inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-colors", mode === "email" ? "bg-white text-slate-800 shadow-sm dark:bg-slate-600 dark:text-slate-100" : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200")}>
        ...
        <div>
          <label htmlFor="block-ta" className="mb-1 block text-sm font-medium text-slate-600 dark:text-slate-300">
            {mode === "email" ? "Email addresses" : "Mobile numbers"} <span className="text-indigo-500">*</span>
          </label>
          <textarea id="block-ta" rows={6} placeholder={mode === "email" ? "guest1@example.com, guest2@example.com" : "+919876543210, +919812345678"} value={textarea} onChange={(e) => setTextarea(e.target.value)} className="block w-full rounded-md border border-slate-200 px-3 py-2 text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 dark:placeholder-slate-500" />
        ...
        <div aria-live="polite" className="mt-2 flex flex-wrap items-center gap-2 text-xs">
          <span className="font-medium text-slate-600 dark:text-slate-300">{parsed.valid.length} {identifierNoun(parsed.valid.length)} ready</span>
        ...
        {chipNumbers.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {chipNumbers.map((c, i) => (
              ...
              }`} title={c.status === "invalid" ? "Invalid format" : c.status === "blocked" ? "Already blocked" : ""}>
                {c.num}
                {c.status !== "blocked" && (
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <button ref={triggerRef} disabled={textarea.trim() === ""} onClick={handlePrimaryClick} className="inline-flex items-center gap-2 rounded-md bg-indigo-600 px-8 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-60">
            Block {parsed.valid.length > 0 ? `${parsed.valid.length} ${identifierNoun(parsed.valid.length)}` : (mode === "email" ? "email addresses" : "numbers")}
          </button>
        </div>
      </CardContent>
      </Card>
      ...
      <CardContent className="p-0">
        {paged.length === 0 ? (
          <EmptyState
            ...
            action={{ label: mode === "email" ? "Block an email" : "Block a number", onClick: () => document.getElementById("block-ta")?.focus() }}
          />
        ) : (
                    ...
                    <TableCell className="font-mono text-xs">
                      ...
                      <span className="inline-flex items-center gap-1.5">
                        {b.identifier.includes("@")
                          ...
                          : <Smartphone className="h-3 w-3 shrink-0 text-slate-400" aria-hidden="true" />}
