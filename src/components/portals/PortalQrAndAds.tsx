...
export function PortalQrPanel({ portal }: { portal: Portal }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm">QR login</CardTitle>
      </CardHeader>
      <CardContent className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            Print or share this QR code so guests can jump straight to the portal without typing a
            URL.
          </p>
        </div>
      </CardContent>
...
export function PortalAdsPanel({ portal }: { portal: Portal }) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Advertisements</CardTitle>
        </CardHeader>
        <CardContent>
          {portal.ads.length === 0 ? (
            <EmptyState
              title="No ads scheduled"
              description="Add banners or promotional videos to appear on the portal."
            />
          ) : (
            <div className="space-y-2">
              {portal.ads.map((ad) => (
                <div key={ad.id} className="flex items-center gap-3 rounded-lg border p-2">
                  {ad.type === "banner" ? (
                    ...
                    <div className="flex h-12 w-24 items-center justify-center rounded bg-muted text-[10px] uppercase tracking-wide text-muted-foreground">
                      Video
                    </div>
                  )}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Schedule an ad</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={submit} className="space-y-3">
            <div className="space-y-1">
              <Label>Name</Label>
              <Input {...register("name")} placeholder="Summer promo" />
            ...
            <div className="space-y-1">
              ...
              <Input {...register("mediaUrl")} placeholder="https://…/banner.jpg" />
              {formState.errors.mediaUrl && (
                <p className="text-xs text-destructive">{formState.errors.mediaUrl.message}</p>
              )}
            </div>
            <div className="space-y-1">
              <Label>Click URL</Label>
              <Input {...register("clickUrl")} placeholder="https://…" />
              {formState.errors.clickUrl && (
                <p className="text-xs text-destructive">{formState.errors.clickUrl.message}</p>
              )}
            </div>
