      ...
      <FieldGrid>
        <div className="space-y-1.5">
          ...
          <Select
            ...
            onValueChange={(v) => patch({ provider: v as SmsSettings["provider"] })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <Field label="Sender ID">
          <Input value={local.senderId} onChange={(e) => patch({ senderId: e.target.value })} />
        </Field>
        <Field label="Template ID">
          <Input value={local.templateId} onChange={(e) => patch({ templateId: e.target.value })} />
        </Field>
        <Field label="API key">
          <PasswordInput value={local.apiKey} onChange={(v) => patch({ apiKey: v })} />
        </Field>
      </FieldGrid>
      {local.lastTestedAt && (
        <p className="text-xs text-muted-foreground">
          Last tested {new Date(local.lastTestedAt).toLocaleString()}
        </p>
      )}
    </SectionCard>
  );
