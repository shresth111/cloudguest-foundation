...
export function ApiPanel({ data }: { data: ApiSettings }) {
  ...
  return (
    <div className="space-y-4">
      <SectionCard title="API keys" description="Rotate and revoke server-to-server credentials.">
        <div className="flex gap-2">
          <Input
            placeholder="Key name (e.g. Analytics ETL)"
            value={name}
      ...
      <SectionCard title="Usage" description="Live API traffic in the last 24h and month-to-date.">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <Stat
            ...
            tone={data.errorsToday > 100 ? "warn" : "ok"}
          />
        </div>
      </SectionCard>
    </div>
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "ok" | "warn" }) {
  return (
    <div className="rounded-lg border border-border/60 bg-card/40 p-3">
      ...
      <div className={`mt-1 text-lg font-semibold ${tone === "warn" ? "text-amber-500" : ""}`}>
        {value}
      </div>
    </div>
  );
