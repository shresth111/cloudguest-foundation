...
const CHANNELS: { key: keyof NotificationSettings; label: string; description: string }[] = [
  {
    ...
    description: "Digest and alert emails to platform admins.",
  },
  ...
  { key: "push", label: "Push notifications", description: "Mobile app push delivery." },
  {
    ...
    description: "In-browser toasts for signed-in admins.",
  },
...
export function NotificationsPanel({ data }: { data: NotificationSettings }) {
  ...
  const save = () =>
    mut.mutate(
      { section: "notifications", value: local },
    ...
    >
      ...
      <FieldGrid>
        ...
        <div className="space-y-1.5">
          ...
          <Input
            value={local.webhookEndpoint}
            onChange={(e) => patch({ webhookEndpoint: e.target.value })}
          />
        </div>
      </FieldGrid>
    </SectionCard>
