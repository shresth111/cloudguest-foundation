...
export function PaymentPanel({ data }: { data: PaymentSettings }) {
  ...
  return (
    ...
    >
      <FieldGrid>
        <div className="space-y-1.5">
          ...
          <Select
            ...
            onValueChange={(v) => patch({ provider: v as PaymentSettings["provider"] })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <Field label="Publishable key">
          <PasswordInput
            value={local.publishableKey}
            onChange={(v) => patch({ publishableKey: v })}
          />
        </Field>
      ...
      <ToggleRow
        label="Auto invoice"
        description="Generate PDF invoices automatically after each charge."
      >
