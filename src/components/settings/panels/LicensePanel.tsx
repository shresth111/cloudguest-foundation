...
export function LicensePanel({ data }: { data: LicenseInfo }) {
  const daysLeft = Math.max(
    0,
    Math.round((new Date(data.expiryDate).getTime() - Date.now()) / 86_400_000),
  );
  const copy = () => {
  ...
  return (
    <SectionCard title="License" description="Plan entitlements and activation details.">
