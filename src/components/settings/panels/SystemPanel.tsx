  ...
  return (
    ...
    >
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <ToggleRow
          label="Debug mode"
          description="Expose stack traces to admins (never in production)."
        >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border/60 bg-card/40 p-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground">
            {local.lastCacheClearAt
              ? `Cache last cleared ${new Date(local.lastCacheClearAt).toLocaleString()}`
              : "No cache clears recorded."}
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <Button size="sm" variant="outline" onClick={restart}>
            <RotateCcw className="mr-2 h-4 w-4" /> Restart services
          </Button>
        </div>
      </div>
    </SectionCard>
