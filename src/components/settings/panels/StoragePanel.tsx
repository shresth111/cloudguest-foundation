        ...
        <FieldGrid>
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(v) => patch({ provider: v as StorageSettings["provider"] })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <Field label="Bucket name">
            <Input value={local.bucket} onChange={(e) => patch({ bucket: e.target.value })} />
          </Field>
          <Field label="Region">
            <Input value={local.region} onChange={(e) => patch({ region: e.target.value })} />
          </Field>
          <Field label="Access key">
            <PasswordInput value={local.accessKey} onChange={(v) => patch({ accessKey: v })} />
          </Field>
          <Field label="Secret key">
            <PasswordInput value={local.secretKey} onChange={(v) => patch({ secretKey: v })} />
          </Field>
        </FieldGrid>
      </SectionCard>
