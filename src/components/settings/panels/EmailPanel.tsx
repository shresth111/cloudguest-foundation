      ...
      <FieldGrid>
        <div className="space-y-1.5">
          ...
          <Select
            ...
            onValueChange={(v) => patch({ provider: v as EmailSettings["provider"] })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <Field label="SMTP host">
          <Input value={local.host} onChange={(e) => patch({ host: e.target.value })} />
        </Field>
        <Field label="Port">
          <Input
            type="number"
  ...
  if (status === "error" || status === "disconnected")
    return (
      <Badge variant="destructive">
        <XCircle className="mr-1 h-3 w-3" /> {status === "error" ? "Error" : "Disconnected"}
      </Badge>
    );
