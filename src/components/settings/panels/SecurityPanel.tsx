...
export function SecurityPanel({ data }: { data: SecuritySettings }) {
  ...
  const save = () =>
    mut.mutate(
      { section: "security", value: local },
      { onSuccess: () => toast.success("Security updated") },
    );
    ...
    <div className="space-y-4">
      ...
      >
        <div className="grid gap-3 md:grid-cols-2">
          <ToggleRow label="Force HTTPS" description="Redirect all traffic to TLS.">
            <Switch checked={local.forceHttps} onCheckedChange={(v) => patch({ forceHttps: v })} />
          </ToggleRow>
          <ToggleRow label="Two-factor auth" description="Require 2FA for admin sign-in.">
            <Switch checked={local.twoFactor} onCheckedChange={(v) => patch({ twoFactor: v })} />
          </ToggleRow>
          <ToggleRow label="Device trust" description="Remember trusted devices for 30 days.">
            <Switch
              checked={local.deviceTrust}
              onCheckedChange={(v) => patch({ deviceTrust: v })}
            />
          </ToggleRow>
          <ToggleRow label="CAPTCHA" description="Show CAPTCHA on repeated failures.">
            <Switch checked={local.captcha} onCheckedChange={(v) => patch({ captcha: v })} />
          </ToggleRow>
          <ToggleRow label="Audit logging" description="Record admin & tenant actions.">
            <Switch
              checked={local.auditLogging}
              onCheckedChange={(v) => patch({ auditLogging: v })}
            />
          </ToggleRow>
          <div className="rounded-lg border border-border/60 bg-card/40 p-3">
            <Label className="text-xs font-medium text-muted-foreground">Password complexity</Label>
            <Select
              value={local.passwordComplexity}
              onValueChange={(v) =>
                patch({ passwordComplexity: v as SecuritySettings["passwordComplexity"] })
              }
            >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <IpListCard
          ...
          onAdd={() => {
            if (!wl.trim()) return;
            patch({ ipWhitelist: [...local.ipWhitelist, wl.trim()] });
            setWl("");
          }}
        ...
        <IpListCard
          ...
          onAdd={() => {
            if (!bl.trim()) return;
            patch({ ipBlacklist: [...local.ipBlacklist, bl.trim()] });
            setBl("");
          }}
      ...
      <div className="flex flex-wrap gap-2">
        {items.length === 0 ? (
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Shield className="h-3.5 w-3.5" /> No entries yet.
          </div>
        ) : (
          items.map((ip) => (
            <Badge
              key={ip}
              variant={tone === "success" ? "secondary" : "destructive"}
              className="gap-1"
            >
              {ip}
              <button
