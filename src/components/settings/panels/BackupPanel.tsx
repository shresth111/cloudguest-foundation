        ...
        <FieldGrid>
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setLocal((s) => ({ ...s, schedule: v as BackupSettings["schedule"] }))
              }
            >
              <SelectTrigger>
                <SelectValue />
          ...
          <Info
            label="Last backup"
            value={data.lastBackupAt ? new Date(data.lastBackupAt).toLocaleString() : "—"}
          />
          <Info
            label="Next backup"
            value={data.nextBackupAt ? new Date(data.nextBackupAt).toLocaleString() : "—"}
          />
        </FieldGrid>
      </SectionCard>
      ...
      <SectionCard title="History" description="Recent backup runs.">
        <div className="space-y-2">
          ...
          {data.history.map((h) => (
            <div
              key={h.id}
              className="flex flex-wrap items-center gap-3 rounded-lg border border-border/60 bg-card/40 p-3"
            >
              ...
              <div className="flex gap-1">
                ...
                <Button size="sm" variant="ghost" onClick={() => restore(h.id)}>
