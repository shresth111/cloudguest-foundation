...
// living beside `Route` in the same file, so `ForgotPasswordPage` could never be split
// away while it lived in a route-registered file. It only ever needed to
// be a named export because `LoginPage` (which renders it inline for "Forgot password?" instead of navigating) imports it cross-file.
//
...
// correctly-split lazy `component` chunks of `/forgot-password` and `/`.
