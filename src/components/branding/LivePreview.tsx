    ...
    <Card>
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center justify-center overflow-auto rounded-xl bg-muted/40 p-4">
          <div
            className={cn(
              "overflow-hidden rounded-xl border shadow-xl transition-all",
              WIDTHS[device],
            )}
            style={{
              fontFamily: brand.typography.fontFamily,
              borderRadius: brand.typography.cardRadius,
            }}
          >
...
function DashboardPreview({ brand, mode }: { brand: Brand; mode: "light" | "dark" }) {
  ...
  return (
    <div style={{ background: bg, color: fg, fontSize: brand.typography.fontSize }}>
      <div className="flex">
        <aside
          className="hidden w-40 shrink-0 p-3 sm:block"
          style={{ background: brand.colors.sidebar, color: "#F8FAFC" }}
        >
          ...
          {["Dashboard", "Guests", "Routers", "Analytics"].map((it, i) => (
            <div
              ...
              style={{ background: i === 0 ? `${brand.colors.primary}30` : "transparent" }}
            >
              {it}
            </div>
          ))}
        ...
        <div className="flex-1">
          <div
            ...
            style={{ background: brand.colors.navbar, color: "#F8FAFC" }}
          >
            <span className="text-xs font-medium">{brand.companyName}</span>
            <div className="flex items-center gap-2">
              <span className="text-[10px] opacity-70">
                admin@{brand.companyName.toLowerCase().replace(/\s+/g, "")}
              </span>
          ...
          <div className="px-4 pb-4">
            <button
              ...
              style={{
                background: brand.colors.buttonBg,
                color: brand.colors.buttonText,
                borderRadius: brand.typography.borderRadius,
                fontWeight: brand.typography.buttonWeight,
              }}
            >
              Primary action
            </button>
...
function LoginPreview({ brand, mode }: { brand: Brand; mode: "light" | "dark" }) {
  ...
  return (
    <div
      className="grid min-h-[380px] grid-cols-1 md:grid-cols-2"
      style={{ background: bg, color: fg, fontSize: brand.typography.fontSize }}
    >
      <div
        className="hidden md:block"
        style={{
          backgroundImage: `url(${brand.login.background})`,
      ...
      <div className="flex flex-col justify-center gap-4 p-6">
        ...
        <div>
          <h3
            ...
            style={{ color: brand.colors.primary, fontWeight: brand.typography.headingWeight }}
          >
            {brand.login.heading}
          </h3>
...
function PortalPreview({ brand, mode }: { brand: Brand; mode: "light" | "dark" }) {
  ...
  return (
    <div
      className="relative"
      style={{ minHeight: 380, background: bg, color: fg, fontSize: brand.typography.fontSize }}
    >
      <div
        className="absolute inset-0 opacity-70"
        style={{
          backgroundImage: `url(${brand.portal.background})`,
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}
      />
      <div
        ...
        style={{ background: `linear-gradient(180deg, transparent, ${bg} 80%)` }}
      />
      <div className="relative flex min-h-[380px] flex-col items-center justify-center gap-3 p-6 text-center">
        ...
        <h3
          ...
          style={{ color: brand.portal.primary, fontFamily: brand.portal.font }}
        >
          {brand.portal.welcomeMessage}
        </h3>
        <p className="max-w-sm text-xs opacity-70">{brand.portal.terms}</p>
        <button
          className="mt-2 px-5 py-2 text-xs"
          style={{
            background: brand.portal.primary,
            color: "#fff",
            borderRadius: brand.typography.borderRadius,
            fontWeight: brand.typography.buttonWeight,
          }}
