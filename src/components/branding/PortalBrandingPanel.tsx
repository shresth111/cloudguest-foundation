...
export function PortalBrandingPanel({ brand }: { brand: Brand }) {
  ...
  const onSubmit = (v: PortalBrandingFormValues) =>
    save.mutate(
      { ...brand, portal: v },
      { onSuccess: () => toast.success("Portal branding saved") },
    );

  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base">Captive portal branding</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 md:grid-cols-2">
          <div>
            <Label className="text-xs">Logo URL</Label>
            <Input className="mt-1" {...form.register("logo")} />
          </div>
          <div>
            <Label className="text-xs">Background URL</Label>
            <Input className="mt-1" {...form.register("background")} />
          </div>
          <ColorField
            label="Primary color"
            register={form.register("primary")}
            value={form.watch("primary")}
            onChange={(v) => form.setValue("primary", v)}
          />
          <ColorField
            label="Accent color"
            register={form.register("accent")}
            value={form.watch("accent")}
            onChange={(v) => form.setValue("accent", v)}
          />
          <div>
            <Label className="text-xs">Font</Label>
            <Input className="mt-1" {...form.register("font")} />
          </div>
          <div>
            <Label className="text-xs">Welcome message</Label>
            <Input className="mt-1" {...form.register("welcomeMessage")} />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <Textarea rows={2} className="mt-1" {...form.register("terms")} />
          </div>
          <div className="md:col-span-2">
            <Label className="text-xs">Privacy policy</Label>
            <Textarea rows={2} className="mt-1" {...form.register("privacy")} />
          </div>
          <div className="md:col-span-2">
            <Button type="submit" disabled={save.isPending}>
              {save.isPending ? "Saving…" : "Save portal branding"}
            </Button>
          </div>
        </form>
      </CardContent>
...
function ColorField({
  ...
  label: string;
  register: ReturnType<ReturnType<typeof useForm<PortalBrandingFormValues>>["register"]>;
  value: string;
  ...
  return (
    <div>
      ...
      <div className="mt-1 flex items-center gap-2">
        <label
          className="h-9 w-10 shrink-0 cursor-pointer overflow-hidden rounded-md border"
