...
export function EmailBrandingPanel({ brand }: { brand: Brand }) {
  ...
  const form = useForm<EmailBrandingFormValues>({
    ...
    defaultValues: {
      header: brand.email.header,
      footer: brand.email.footer,
      companyLogo: brand.email.companyLogo,
      companyAddress: brand.email.companyAddress,
    },
  });
  ...
  useEffect(
    ...
    [brand.id, brand.email, form],
  );

  const onSubmit = (v: EmailBrandingFormValues) => {
    save.mutate(
      { ...brand, email: { ...brand.email, ...v } },
  ...
  const updateSocial = (key: "twitter" | "linkedin" | "facebook" | "instagram", value: string) => {
    save.mutate({
      ...brand,
      email: { ...brand.email, socials: { ...brand.email.socials, [key]: value } },
    });
  };
        ...
        <CardContent>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <Input className="mt-1" {...form.register("header")} />
            </div>
            <div>
              <Label className="text-xs">Company logo URL</Label>
              <Input className="mt-1" {...form.register("companyLogo")} />
            </div>
            <div className="md:col-span-2">
              <Label className="text-xs">Footer</Label>
              <Textarea rows={2} className="mt-1" {...form.register("footer")} />
            </div>
            <div className="md:col-span-2">
              <Label className="text-xs">Company address</Label>
              <Input className="mt-1" {...form.register("companyAddress")} />
            </div>
              ...
              {(["twitter", "linkedin", "facebook", "instagram"] as const).map((k) => (
                <div key={k}>
                  ...
                  <Input
                    ...
                    onBlur={(e) =>
                      e.target.value !== (brand.email.socials[k] ?? "") &&
                      updateSocial(k, e.target.value)
                    }
                  />
                </div>
              ))}
            ...
            <div className="md:col-span-2">
              <Button type="submit" disabled={save.isPending}>
                {save.isPending ? "Saving…" : "Save email branding"}
              </Button>
            </div>
          </form>
        </CardContent>
