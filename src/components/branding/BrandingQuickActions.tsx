...
export function BrandingQuickActions({ brand }: { brand: Brand }) {
  ...
  const handleImport = async (file: File) => {
    ...
    importTheme.mutate(
      { id: brand.id, payload: text },
      {
        onSuccess: (ok) =>
          ok ? toast.success("Theme imported") : toast.error("Invalid theme file"),
      },
    );
  };
  ...
  const actions = [
    {
      ...
      onClick: () => save.mutate(brand, { onSuccess: () => toast.success("Branding saved") }),
    },
    {
      label: "Publish",
      icon: Rocket,
      ...
      onClick: () =>
        publish.mutate(brand.id, { onSuccess: () => toast.success("Brand published live") }),
    },
    {
      label: "Reset theme",
      icon: RotateCcw,
    ...
    },
    {
      ...
      onClick: () =>
        duplicate.mutate(brand.id, { onSuccess: () => toast.success("Brand duplicated") }),
    },
    {
      label: "Export theme",
      icon: Download,
