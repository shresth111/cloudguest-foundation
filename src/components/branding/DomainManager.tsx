...
export function DomainManager({ brand, domain }: { brand: Brand; domain?: CustomDomain }) {
  ...
  const onSubmit = (v: DomainFormValues) => {
    add.mutate(
      { brandId: brand.id, domain: v.domain },
      {
        onSuccess: () => {
        ...
        <CardContent className="space-y-4">
          {domain ? (
            ...
            <EmptyState
              ...
              description="Add a domain to serve the portal and admin dashboard under your brand."
            />
          )}
          ...
          >
            <div className="flex-1 min-w-[220px]">
              ...
              <Input
                className="mt-1"
                placeholder="wifi.brandname.com"
...
function StatusRow({
  ...
  icon: typeof Globe;
  label: string;
  state: "issued" | "verified" | "active" | "pending" | "verifying" | "failed";
}) {
  return (
