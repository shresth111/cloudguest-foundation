...
export function GuestTeamManagement() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(
      (t) => t.name.toLowerCase().includes(s) || t.teamCode.toLowerCase().includes(s),
    );
  }, [teams.data, search]);
                ...
                {filtered.map((t) => (
                  <TableRow key={t.id} className="group">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1.5">
                        ...
                        <Button
                          ...
                          className="h-6 w-6"
                          onClick={() => {
                            navigator.clipboard.writeText(t.teamCode);
                    ...
                    <TableCell className="text-sm">
                      {t.sharedDataLimitMb ? `${t.sharedDataLimitMb} MB` : "—"}
                    </TableCell>
                    <TableCell>
...
function TeamDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>New guest team</DialogTitle>
        </DialogHeader>
        <form
          className="space-y-4"
          onSubmit={form.handleSubmit(async (v) => {
            try {
              await create.mutateAsync({
                ...
                sharedDataLimitMb:
                  v.sharedDataLimitMb === "" ? undefined : Number(v.sharedDataLimitMb),
