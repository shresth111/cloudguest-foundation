      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Rate catalog used to compute every invoice's tax. For India (country code IN), the
              backend automatically splits this into CGST + SGST when the buyer is in the same state
              as the platform, or IGST when they're in a different state.
            </p>
          </div>
        ...
        <CardContent>
          ...
          <div className="mt-4 flex items-start gap-2 rounded-lg border border-dashed p-3 text-xs text-muted-foreground">
            ...
            Each organization's GSTIN and billing state (needed to decide CGST/SGST vs IGST) is set
            on its own billing profile, not here — this catalog only defines the rate percentages
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => form.setValue("taxType", v as TaxType)}
            >
          ...
          <div>
            ...
            <Input
              ...
              step="0.01"
              className="mt-1"
              {...form.register("ratePercentage", { valueAsNumber: true })}
            />
            {form.formState.errors.ratePercentage && (
              <p className="mt-1 text-xs text-destructive">
                {form.formState.errors.ratePercentage.message}
              </p>
            )}
          </div>
          ...
          <DialogFooter className="col-span-2">
            ...
            <Button type="submit" disabled={save.isPending}>
              {save.isPending ? "Saving…" : "Save tax rate"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
