        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Automate delivery of billing reports to stakeholders.
            </p>
          </div>
          <Button size="sm" onClick={() => setCreating(true)}>
            <Plus className="mr-1.5 h-4 w-4" /> Schedule report
          </Button>
        </CardHeader>
        <CardContent className="space-y-3">
          {list.isLoading ? (
            ...
            <EmptyState
              ...
              description="Set up recurring email delivery of billing reports."
              action={{ label: "Schedule report", onClick: () => setCreating(true) }}
            />
          ) : (
            list.data.map((r) => (
              <div
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground capitalize">
                        {r.frequency} · {r.format.toUpperCase()} · Next:{" "}
                        {dateFmt.format(new Date(r.nextRunAt))}
                      </div>
                    </div>
                  </div>
                  <div className="mt-2 flex flex-wrap gap-1">
                    {r.recipients.map((e) => (
                      <Badge key={e} variant="outline" className="gap-1 font-normal">
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Schedule billing report</DialogTitle>
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => form.setValue("frequency", v as ReportFrequency)}
              >
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => form.setValue("format", v as BillingReportFormat)}
              >
                <SelectTrigger className="mt-1">
                  <SelectValue />
                </SelectTrigger>
