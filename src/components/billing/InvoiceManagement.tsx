...
import { useCreateManualInvoice, useDownloadInvoice, useOrganizationsList } from "@/hooks/useBilling";
...
export function InvoiceManagement({ data, subscriptions, isLoading, isError, onRetry }: Props) {
  ...
  const orgsFromSubscriptions = (subscriptions ?? []).map((s) => ({ id: s.organizationId, name: s.organizationName }));
  const allOrgs = Array.from(
    new Map([...orgsFromSubscriptions, ...(orgsWithoutSubscription.data ?? [])].map((o) => [o.id, o.name])).entries(),
  ).map(([id, name]) => ({ id, name }));
  ...
  const canSubmit = !!generateOrgId && parsedLineItems.length > 0 && parsedLineItems.length === lineItems.length;
  ...
  function handleGenerateAndSend() {
    createManualInvoice.mutate(
      { organizationId: generateOrgId, lineItems: parsedLineItems },
      {
        ...
        onError: (err) => toast.error((err as unknown as AppError).message || "Could not generate the invoice -- check the organization has a billing profile on file."),
      },
    );
  }
  ...
  return (
    <>
      ...
      <Dialog open={generateOpen} onOpenChange={(o) => { setGenerateOpen(o); if (!o) resetGenerateForm(); }}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>New invoice</DialogTitle>
            <DialogDescription>
              Bills exactly the line items below (not the organization's subscription price) -- real GST is computed on the subtotal, and the PDF is emailed to their billing contact. Requires a billing profile on file for the organization.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setLineItems((prev) => [...prev, blankLineItem()])}>
          ...
          <DialogFooter>
            ...
            <Button disabled={!canSubmit || createManualInvoice.isPending} onClick={handleGenerateAndSend}>
              {createManualInvoice.isPending ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <Mail className="mr-1.5 h-4 w-4" />}
              Generate &amp; send
            </Button>
          </DialogFooter>
        </DialogContent>
