 ...
 * (`/customer`, `/customer/users`, `/customer/$feature`) -- mirrors
 * `requireCustomerSession`'s (`src/lib/authGuards.ts`) "throw redirect()
 * from beforeLoad" shape, but for location context instead of auth.
 * `activeLocationId` lives in `useCustomerStore`, a plain Zustand store
 * (persisted to localStorage), not router context, so it's read via the
 * store's own static `.getState()` rather than a hook -- `beforeLoad` runs
 * outside a component, before anything renders.
 *
 * A visitor with no active location (first login before the picker has
 * ever run, or a cleared/expired store) gets bounced to `/customer/locations`,
 ...
 * object, not just the id (the picker at `/customer/locations` stores
 * both, see `customer.locations.tsx`'s own `handleSelect`), so a
 * direct-hit on a long URL
