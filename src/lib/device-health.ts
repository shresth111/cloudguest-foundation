...
/**
 * Interfaces that must never reach a customer-facing surface.
 *
 * THE HARD CONSTRAINT, AND THE LIVE BREACH IT CLOSES
 * --------------------------------------------------
 * Nothing customer-facing may expose WireGuard, RADIUS, tunnels or NAS
 * internals. That is not a style rule -- it is the one non-negotiable in this
 * product. It was being broken in production: "Traffic by port" on the venue
 * owner's own Devices screen listed **`wg-cloudguard` — Up, 258 KB in / 631 KB
 * out**, beside `ether1`…`ether5` and `bridge`. That is the management tunnel,
 * named, with its byte counters, on a screen a cafe owner opens.
 *
 * Nothing filtered it because nothing was looking: the SNMP sweep reports
 * every interface the device has, and this module faithfully charted all of
 * them. The device is not wrong to have the interface and the API is not wrong
 * to report it; the CONSOLE is wrong to draw it for a customer.
 *
 * MATCHED BY SHAPE, NOT BY EXACT NAME. A denylist of one literal string is one
 * rename away from leaking again, and these names are generated by our own
 * provisioning templates, which have already changed once. So: anything that
 * looks like a WireGuard interface (`wg…`), anything carrying the platform's
 * own tunnel naming, and the generic tunnel interface families RouterOS
 * offers. Over-matching here costs a venue owner one port they did not ask
 * about; under-matching costs the constraint.
 *
 * Applied in `toInterfaceSeries`, which is the single funnel every consumer
 * of per-interface traffic goes through -- rather than in the one component
 * where it was spotted, which is how the next surface inherits the leak.
 */
const TUNNEL_INTERFACE_PATTERNS: readonly RegExp[] = [
  /^wg[-_]?/i, // wg-cloudguard, wg0, wg_mgmt
  /cloudguard/i, // whatever prefix a future template gives it
  /^(gre|ipip|eoip|l2tp|pptp|sstp|ovpn|vpn|tun|tap)[-_0-9]/i,
  /^(gre|ipip|eoip|l2tp|pptp|sstp|ovpn|vpn|tun|tap)$/i,
];

/**
 * Whether this interface is management/tunnel plumbing rather than one of the
 * venue's own ports.
 *
 * Exported so a test can assert the exact names seen in production, and so a
 * surface that legitimately shows tunnels (the Master console, never the
 * customer dashboard) can ask the same question rather than inventing a
 * second list.
 */
export function isTunnelInterfaceName(ifName: string | null | undefined): boolean {
  const name = (ifName ?? "").trim();
  if (!name) return false;
  return TUNNEL_INTERFACE_PATTERNS.some((re) => re.test(name));
}
...
export function toInterfaceSeries(readings: DeviceHealthReading[]): InterfaceSeries[] {
  ...
  for (const reading of withCounters) {
    for (const counter of reading.interfaceTrafficCounters ?? []) {
      // The management tunnel is not one of the venue's ports, and naming it
      // on a customer screen breaks the product's one hard constraint. Dropped
      // before the series exists, so no downstream consumer -- chart, peak
      // callout, totals, CSV export -- can reintroduce it.
      if (isTunnelInterfaceName(counter.ifName)) continue;
      const k = key(counter);
