 ...
 * already set a real password. Used purely to pick the sign-in card's
 * *default* tab -- "OTP once, then phone/email + password from then on"
 * (see src/components/portal-runtime/GuestSignInCard.tsx) -- never to
 * gate or skip an actual auth call: every real login still goes through
 * the real backend regardless of this flag, and an explicit tab click or
 * an incoming `selectedMethod` (e.g. the expired screen's "Use OTP
 * instead") always overrides it.
 *
