...
import { Menu, LogOut, EyeOff, ChevronsUpDown, ShieldCheck, Inbox, KeyRound } from "lucide-react";
  ...
  const groups = useMemo(
    () =>
      FEATURE_GROUPS.map((g) => ({
        group: g.group,
        items: g.items.filter((i) => granted.has(i.id)),
      ...
      <div className="flex min-h-screen items-center justify-center bg-muted/30 p-6 text-center">
        <div className="max-w-sm space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            No staff profile is configured. Ask your business owner to create your staff account
            under <span className="font-medium text-foreground">Staff Access</span>.
          </p>
        </div>
      </div>
    );
      ...
      >
        ...
        <div className="border-t p-3 text-[11px] text-muted-foreground">
          <span className="inline-flex items-center gap-1">
            <ShieldCheck className="h-3.5 w-3.5 text-primary" /> {granted.size} features granted
          </span>
        </div>
      </aside>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuItem onClick={() => setTfaOpen(true)}>
