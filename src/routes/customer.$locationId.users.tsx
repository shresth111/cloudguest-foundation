...
interface User { id: string; name: string; email: string; device: string; mac: string; ip: string; duration: string; download: string; status: "online" | "offline" | "idle"; }

const ALL_USERS: User[] = Array.from({ length: 24 }, (_, i) => ({
  ...
  mac: `00:1A:${String(10 + i)}:${String(i).padStart(2, "0")}`, ip: `10.0.${Math.floor(i / 8) + 1}.${100 + i}`,
  duration: `${15 + (i % 6) * 10} min`, download: `${(Math.random() * 500).toFixed(0)} MB`,
  status: i < 16 ? "online" : i < 20 ? "idle" : "offline",
}));
...
function CustomerUsersPage() {
  ...
  const toggleTheme = () => { setTheme((p) => p === "light" ? "dark" : "light"); document.documentElement.classList.toggle("dark"); };
  ...
  const filtered = useMemo(() => {
    ...
    if (statusTab !== "all") items = items.filter((u) => u.status === statusTab);
    return items;
  }, [search, statusTab]);
  ...
  const toggleAll = () => { if (selected.size === pageItems.length) setSelected(new Set()); else setSelected(new Set(pageItems.map((u) => u.id))); };
    ...
    <div className={cn("customer-theme flex min-h-screen", theme === "dark" && "dark")}>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b-2 border-border bg-card px-4">
          ...
          <div className="relative"><button onClick={() => setNotifOpen(!notifOpen)} className="flex h-8 w-8 items-center justify-center text-muted-foreground relative"><Bell className="h-4 w-4" /><span className="absolute -right-0.5 -top-0.5 h-3 w-3 rounded-full bg-[#ec3013] text-[7px] font-bold text-white flex items-center justify-center">3</span></button></div>
        ...
        <main className="flex-1 bg-[#f3f2f2] dark:bg-neutral-900 p-6">
          <div className="max-w-7xl mx-auto space-y-4">
            ...
            <div className="flex gap-0 border-b-2 border-border">
              {["all", "online", "offline"].map((tab) => (
                <button key={tab} onClick={() => { setStatusTab(tab); setPage(0); }} className={cn("px-4 py-2 text-xs font-semibold uppercase border-b-2 -mb-[2px]", statusTab === tab ? "border-[#ec3013] text-[#ec3013]" : "border-transparent text-muted-foreground")}>
                  {tab}<Badge variant="outline" className="ml-2 h-4 px-1 text-[9px]">{filtered.length}</Badge>
                </button>
              ))}
            </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/30">
                    <TableHead className="w-10"><button onClick={toggleAll}>{selected.size === pageItems.length && pageItems.length > 0 ? <CheckSquare className="h-4 w-4 text-[#ec3013]" /> : <Square className="h-4 w-4" />}</button></TableHead>
                ...
                <TableBody>
                  {pageItems.length === 0 ? (<TableRow><TableCell colSpan={8} className="py-12 text-center text-sm text-muted-foreground">No users found</TableCell></TableRow>)
                  : pageItems.map((u) => (
                    <TableRow key={u.id} className="border-b border-border">
                      ...
                      <TableCell className="text-right"><Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setDrawerUser(u)}><Eye className="h-3.5 w-3.5" /></Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            ...
            {totalPages > 1 && <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">{filtered.length} users</span><div className="flex items-center gap-1">
            ...
            {drawerUser && <div className="fixed inset-0 z-50 flex justify-end">
              <div className="w-full max-w-lg border-l-2 border-border bg-card flex flex-col">
                <div className="flex items-center justify-between p-4 border-b-2 border-border"><div><p className="font-semibold">{drawerUser.name}</p><p className="text-xs text-muted-foreground">{drawerUser.email}</p></div><Button variant="ghost" size="sm" onClick={() => setDrawerUser(null)}>✕</Button></div>
                <div className="flex gap-0 border-b-2 border-border px-4">
                  {["overview", "login", "sessions", "devices", "otp"].map((t) => (<button key={t} className="px-3 py-2 text-xs font-medium capitalize border-b-2 -mb-[2px] border-transparent text-muted-foreground">{[t]}</button>))}
                </div>
                <div className="flex-1 overflow-y-auto p-4">
                  <div className="grid grid-cols-2 gap-3">
                    {[{ label: "MAC", value: drawerUser.mac }, { label: "IP", value: drawerUser.ip }, { label: "Device", value: drawerUser.device }, { label: "Duration", value: drawerUser.duration }, { label: "Download", value: drawerUser.download }].map((f) => (<div key={f.label} className="border-2 border-border p-3"><p className="text-xs text-muted-foreground">{f.label}</p><p className="mt-0.5 text-sm font-medium">{f.value}</p></div>))}
                  </div>
                  <div className="flex gap-2 mt-4"><Button size="sm" className="flex-1 bg-[#ec3013] hover:bg-[#d42a11] text-white"><XCircle className="mr-2 h-4 w-4" />Disconnect</Button><Button variant="outline" size="sm" className="flex-1 border-2 border-border"><Ban className="mr-2 h-4 w-4" />Blacklist</Button></div>
                </div>
              </div>
            </div>}
          </div>
