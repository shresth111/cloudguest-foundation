...
function ExportsPage() {
  ...
  const trigger = async (entity: string, format: Fmt) => {
    ...
    setHistory((h) =>
      [{ id: `e_${Date.now()}`, entity, format, at: new Date().toISOString() }, ...h].slice(0, 20),
    );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Export center"
        description="Bulk download data in PDF, Excel, or CSV formats."
      />
          ...
          return (
            <Card key={e.key}>
              ...
              <CardContent className="flex flex-wrap gap-2">
                <Button variant="outline" size="sm" onClick={() => trigger(e.label, "pdf")}>
                  <FileDown className="mr-1 h-4 w-4" />
                  PDF
                </Button>
                <Button variant="outline" size="sm" onClick={() => trigger(e.label, "excel")}>
                  <FileSpreadsheet className="mr-1 h-4 w-4" />
                  Excel
                </Button>
                <Button variant="outline" size="sm" onClick={() => trigger(e.label, "csv")}>
                  <FileDown className="mr-1 h-4 w-4" />
                  CSV
                </Button>
              </CardContent>
