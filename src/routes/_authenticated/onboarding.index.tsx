          ...
          <motion.div key={step} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
            ...
            {step === 1 && (
              <div className="rounded-xl border border-dashed border-border p-10 text-center">
                ...
                <Button className="mt-4" onClick={() => toast.success("Logo uploaded")}>
                  Choose file
                </Button>
              </div>
            )}
            ...
            {step === 3 && (
              <div>
                ...
                />
                <div
                  className="mt-4 rounded-xl border border-border p-6 text-center"
                  style={{
                    background: `linear-gradient(135deg, ${state.primary}22, ${state.accent}22)`,
                  }}
                >
                  <p className="text-lg font-semibold" style={{ color: state.primary }}>
                    {state.portalName}
                  </p>
                  <p className="mt-1 text-sm text-muted-foreground">
            ...
            {step === 5 && (
              <div>
                ...
                <Input
                  placeholder="alice@acme.io, bob@acme.io"
                  value={state.team}
                  onChange={(e) => set("team", e.target.value)}
                />
                <p className="mt-1 text-xs text-muted-foreground">
                  You can invite more people later from Users & Roles.
                </p>
              </div>
            )}
            {step === 6 && (
              <div className="rounded-xl border border-border bg-gradient-to-br from-primary/10 via-background to-background p-8 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  All defaults are saved. Start onboarding guests now.
                </p>
              </div>
            )}
          </motion.div>
