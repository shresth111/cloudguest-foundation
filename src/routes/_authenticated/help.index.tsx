...
const QUICK = [
  { icon: LifeBuoy, title: "Support tickets", desc: "Open and track cases.", cta: "Open ticket" },
  {
    icon: MessageCircle,
    title: "Live chat",
    desc: "Chat with an engineer, 24/7.",
    cta: "Start chat",
  },
...
function HelpPage() {
  ...
  const filtered = useMemo(
    () =>
      (q.data ?? []).filter(
        (a) => !search || `${a.title} ${a.summary}`.toLowerCase().includes(search.toLowerCase()),
      ),
    [q.data, search],
  );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Help center"
        description="Documentation, tutorials, and support — all in one place."
      />
            ...
            <Card key={q.title}>
              <CardContent className="pt-6">
                ...
                <Button
                  ...
                  size="sm"
                  className="mt-4"
                  onClick={() => toast.success(`${q.cta} coming up`)}
                >
                  {q.cta}
                </Button>
              </CardContent>
