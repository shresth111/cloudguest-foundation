  ...
  async function updateStatus(t: SupportTicket, status: TicketStatus) {
    ...
    try {
      const updated = await ticketService.update(t.id, {
        status,
        resolutionNotes:
          status === "resolved" || status === "closed" ? resolution || undefined : undefined,
      });
      ...
      <MPageShell>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <MStat
            ...
            delta={urgentOpen ? `${urgentOpen} urgent` : undefined}
            icon={LifeBuoy}
            accent={openCount > 0}
          />
          <MStat
          ...
          {!loading &&
            rows.map((t) => (
              <MTr key={t.id} onClick={() => setSelected(t)}>
                ...
                <MTd>
                  <MTag
                    label={TICKET_STATUS_LABEL[t.status]}
                    tone={
                      t.status === "open"
                        ? "open"
        ...
        <MDrawer
          ...
          subtitle={
            selected
              ? `${orgNames.get(selected.organizationId) ?? "—"} · raised by ${selected.createdByName}`
              : ""
          }
          footer={
            selected && (
          ...
          {selected && (
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg border border-border p-3">
                  ...
                  <div className="mt-1.5">
                    <MTag label={TICKET_PRIORITY_LABEL[selected.priority]} />
                  </div>
                </div>
                <div className="rounded-lg border border-border p-3">
                  <p className="text-xs font-medium text-muted-foreground">Status</p>
                  <div className="mt-1.5">
                    <MTag label={TICKET_STATUS_LABEL[selected.status]} />
                  </div>
                </div>
                <div className="rounded-lg border border-border p-3">
              ...
              <div>
                ...
                <p className="text-sm">
                  {selected.createdByName} · {selected.createdByEmail}
                </p>
              </div>
                ...
                <div className="max-h-72 space-y-2 overflow-y-auto rounded-lg border border-border bg-muted/30 p-3">
                  {repliesLoading ? (
                    ...
                    replies.map((r) => (
                      <div
                        key={r.id}
                        className={`rounded-lg border p-2.5 text-sm ${r.isStaffReply ? "border-primary/30 bg-primary/5" : "border-border bg-card"}`}
                      >
