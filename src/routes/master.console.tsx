...
import { MSectionHeader, MField, M_INPUT, MButton, MTag, MDialog } from "@/components/master/MasterKit";
...
function DeviceConsoleScreen() {
  ...
  const runCommand = async () => {
    ...
    try {
      ...
      setHistory((h) => [
        { id: entryId, routerName: selectedRouter.name, command: trimmed, at: new Date().toISOString(), result, error: null },
        ...h,
      ]);
      ...
      setHistory((h) => [
        { id: entryId, routerName: selectedRouter.name, command: trimmed, at: new Date().toISOString(), result: null, error: message },
        ...h,
      ]);
      toast.error(message);
    } finally {
  ...
  return (
    <MasterShell title="Device Console">
      ...
      <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
        ...
        <p>
          This runs a raw RouterOS command directly on the selected device over its real management
          connection — there is no undo, and no whitelist of "safe" commands. Every command run here is
          audited. Use with the same care you would use Winbox's own terminal.
        </p>
      </div>
              ...
              <div key={h.id} className="rounded-lg border border-border p-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {h.error ? (
                    ...
                    <MTag label={`exit ${h.result?.exitStatus}`} tone={h.result?.exitStatus === 0 ? "online" : "degraded"} />
                  )}
                </div>
                ...
                {h.error ? (
                  ...
                  <>
                    {h.result?.stdout && (
                      <pre className="mt-2 whitespace-pre-wrap rounded-md bg-muted p-2 text-xs">{h.result.stdout}</pre>
                    )}
                    {h.result?.stderr && (
                      <pre className="mt-2 whitespace-pre-wrap rounded-md bg-rose-50 p-2 text-xs text-rose-700 dark:bg-rose-500/10 dark:text-rose-400">
                        {h.result.stderr}
        ...
        <div className="space-y-2 rounded-xl border border-border bg-card p-4 shadow-sm">
          ...
          {selectedRouter ? (
            <div className="space-y-1 text-sm">
              ...
              <p className="text-xs text-muted-foreground">{selectedRouter.model} · {selectedRouter.vendor}</p>
              <p className="text-xs text-muted-foreground">{selectedRouter.locationName} / {selectedRouter.organizationName}</p>
              <p className="font-mono text-xs text-muted-foreground">
                {selectedRouter.managementIpAddress ?? selectedRouter.publicIpAddress ?? "no IP on file"}
              </p>
              <MTag label={selectedRouter.status} />
            </div>
          ) : (
            <p className="text-xs text-muted-foreground">Pick a router to see its details here.</p>
          )}
        </div>
      </div>
      ...
      <MDialog
        ...
        footer={
          <>
            <MButton variant="outline" onClick={() => setConfirming(false)}>Cancel</MButton>
      ...
      >
        <div className="space-y-3 text-sm">
          <p className="text-muted-foreground">
            This will run immediately on <span className="font-semibold text-foreground">{selectedRouter?.name}</span>,
            with no confirmation on the device side.
          </p>
          <pre className="whitespace-pre-wrap rounded-md bg-muted p-3 font-mono text-xs">{command}</pre>
        </div>
      </MDialog>
    </MasterShell>
  );
