      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <CustomerHeader
          title={
            ...
            <p className="truncate text-sm font-semibold capitalize">
              {feature === "dashboard" ? "Dashboard" : feature === "admin-logs" ? "Logs" : feature}{" "}
              · {activeLocation?.name ?? ""}
            </p>
          }
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
          <div className="mx-auto max-w-7xl">
            ...
            {feature === "short-links" && <ShortLinksPage locationId={locationId} />}
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {[
        ...
        <Card key={r.n} className="premium-card premium-card-hover cursor-pointer">
          <CardContent className="p-5">
            <p className="font-semibold">{r.n}</p>
            <p className="text-xs text-muted-foreground mt-1">Last generated 3d ago</p>
            <div className="flex gap-2 mt-3">
              <Button
                size="sm"
                variant="outline"
                className="h-8 text-xs"
                onClick={() => toast.success(`${r.n} exported`)}
              >
                PDF
              </Button>
              <Button
