...
function CustomerHomePage() {
  ...
  const handleSelect = (loc: CustomerLocationSummary) => {
    setActiveLocation(loc.id, loc as any);
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="border-2 border-border bg-card p-5 animate-pulse">
                <div className="h-10 w-10 bg-muted mb-3" />
    ...
    return (
      <div className="min-h-screen bg-[#f3f2f2] dark:bg-neutral-900 p-6 customer-theme">
        <div className="max-w-6xl mx-auto text-center py-20">
          ...
          <Button onClick={() => refetch()} variant="outline" className="border-2 border-border rounded-none">
                ...
                <div className="mt-4 space-y-2 border-t-2 border-border pt-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="flex items-center gap-1.5">
                      <span className={cn("h-2 w-2 rounded-full", loc.status === "online" ? "bg-emerald-500" : loc.status === "degraded" ? "bg-amber-500" : "bg-rose-500")} />
