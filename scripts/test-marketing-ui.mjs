...
const stored = {
  ...
  display: { route: "4", sender_id: "ACMECF", dlt_entity_id: "1101", api_key: { set: true, hint: "…c81d" } },
};
...
check("non-secret fields are prefilled from display", init.sender_id === "ACMECF" && init.route === "4");
check("the secret placeholder shows only the hint", P.secretPlaceholder(stored.display.api_key) === "Saved (…c81d). Leave blank to keep.");
const keep = P.buildProviderPut(ping, { ...init }, stored);
check("an untouched form sends an empty config (nothing to change)", keep.body && Object.keys(keep.body.config).length === 0, JSON.stringify(keep));
const edit = P.buildProviderPut(ping, { ...init, sender_id: "ACMEXY" }, stored);
check("an edit sends only the changed field, and omits the blank secret", JSON.stringify(edit.body?.config) === '{"sender_id":"ACMEXY"}', JSON.stringify(edit.body));
...
const create = P.buildProviderPut(ping, { route: "4", sender_id: "X", dlt_entity_id: "1", api_key: "" }, null);
check("creating without the secret is refused client-side", create.body === null && create.errors.api_key === "Required.");
const smtp = P.providerTypeDef("smtp");
const smtpNew = P.buildProviderPut(
  smtp,
  { host: "mail.acme.in", port: "587", use_tls: "true", username: "u", password: "p4ssword-long", from_address: "a@acme.in", from_name: "Acme", reply_to: "" },
  null,
);
check("port is sent as a number and TLS as a boolean", smtpNew.body?.config.port === 587 && smtpNew.body?.config.use_tls === true);
check("switching provider type requires every secret again", P.buildProviderPut(P.providerTypeDef("exotel"), { account_sid: "a", subdomain: "api.exotel.com", sender_id: "s", dlt_entity_id: "d", api_key: "", api_token: "" }, stored).body === null);
check("WhatsApp (meta_cloud) is listed but not offered until BE-11b", P.providerTypeDef("meta_cloud").available === false);

check("no own row -> no acknowledgement", !P.needsFallbackAcknowledgement({ provider_source: "wyfy", own_provider_status: null }));
check("own row failed -> acknowledgement required", P.needsFallbackAcknowledgement({ provider_source: "wyfy", own_provider_status: "failed" }));
check("own row effective -> no acknowledgement", !P.needsFallbackAcknowledgement({ provider_source: "own", own_provider_status: "verified" }));
check("an older backend without the fields -> no acknowledgement", !P.needsFallbackAcknowledgement({}));

const hooksSrc2 = strip(read("src/hooks/useMarketing.ts"));
check(
  "acknowledge_wyfy_fallback is sent only when ticked",
  /\.\.\.\(v\.acknowledgeWyfyFallback \? \{ acknowledge_wyfy_fallback: true \} : \{\}\)/.test(hooksSrc2),
);
...
check(
  ...
  /const canGo =\s*\(!needsAck \|\| ack\) &&/.test(sched) && /acknowledgeWyfyFallback: needsAck && ack/.test(sched),
);
check("provider writes are never optimistic", !/onMutate/.test(hooksSrc2) && /invalidate\("providers", "status", "templates"\)/.test(hooksSrc2));
const form = strip(read("src/components/marketing/channels/ProviderForm.tsx"));
check("secret inputs are password fields with no autofill of the stored value", /type=\{f\.secret \? "password"/.test(form) && /autoComplete=\{f\.secret \? "new-password"/.test(form));
check("the form never reads a secret from display into state", /initialProviderValues\(def, storedDisplay\)/.test(form) && !/display\[f\.key\]/.test(form));
const helpersSrc = strip(read("src/components/marketing/marketing-helpers.ts"));
check("a BYO 402 is told apart from the Marketing lock by feature_key", /feature_key === "guest_marketing_byo"/.test(helpersSrc));
const viewSrc = strip(read("src/components/marketing/MarketingView.tsx"));
check("the Marketing page lock ignores the BYO lock (status is not BYO-gated)", !/isByoLocked/.test(viewSrc));
const svc2 = read("src/services/marketing.service.ts");
for (const path of ['"/marketing/providers"', "/marketing/providers/${", "/verify`", "/marketing-providers`"]) {
  check(`client calls ${path.replace(/["`$]/g, "")}`, svc2.includes(path));
}
const panel = strip(read("src/components/master/CustomerAddonsPanel.tsx"));
check("Master: a blocked add-on's switch is disabled", /disabled=\{!canWrite \|\| busy \|\| !!addon\.blocked_by\}/.test(panel));
check("Master: the providers list shows no hints (reads only sender_label)", /sender_label/.test(panel) && !/display\./.test(panel) && !/hint/.test(panel));
...
check(
  ...
  /useSupportRequest\(MARKETING_REQUEST_SUBJECT\)/.test(upsell) &&
    /async requestSupport\([\s\S]{0,300}ticketService\.create\(/.test(read("src/services/marketing.service.ts")),
);
