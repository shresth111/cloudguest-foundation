...
const P = (b) =>
  ...
  encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24">${b}</svg>`);
const PHOTO = P('<rect width="24" height="24" fill="#8A6E52"/>');
const WIDE =
  "data:image/svg+xml;utf8," +
  encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 480 96"><rect width="480" height="96" fill="#0F3D2E"/><text x="240" y="60" font-size="48" fill="#E8D8A8" text-anchor="middle">ASHOKA PALACE</text></svg>',
  );
const msgs = {
  none: null,
  one: "Complimentary WiFi for our guests.",
  three:
    "Welcome to our home away from home. Complimentary high-speed WiFi is included with your stay, and our front desk is here to help.",
...
const cfg = (o) => ({
  ...
  terms_and_conditions_text: "Fair use.",
...
const U =
  "/portal/welcome?organizationId=11111111-1111-4111-8111-111111111111&locationId=22222222-2222-4222-8222-222222222222&routerId=33333333-3333-4333-8333-333333333333";
...
async function open(vw, vh, o, scale) {
  ...
  await p.route("**/captive-portal/resolve*", (r) =>
    r.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(cfg(o)) }),
  );
...
for (const [vw, vh] of [
  ...
  for (const scale of ["1", "1.25"])
    for (const [k, m] of Object.entries(msgs)) {
      ...
      console.log(
        `${vw}x${vh} ${k.padEnd(7)} ${scale.padEnd(6)} ${String(box ? Math.round(box.y) : "-").padEnd(7)} ${String(box ? Math.round(box.y + box.height) : "-").padEnd(7)} ${String(vh).padEnd(5)} ${box ? (box.y + box.height <= vh ? "visible" : "BELOW FOLD") : "NO BTN"}`,
      );
      await ctx.close();
    }
...
console.log("\n=== DEVANAGARI: pg-title line boxes vs ink at 1.15 line-height ===");
{
  const { ctx, p } = await open(
    390,
    844,
    { name: "होटल श्री महाराजा पैलेस एवं सम्मेलन केंद्र", default_language: "hi" },
    "1.25",
  );
  ...
  console.log(
    await h1.evaluate((el) => {
      ...
      const rects = [...r.getClientRects()].map((x) => ({
        top: +x.top.toFixed(1),
        bot: +x.bottom.toFixed(1),
        h: +x.height.toFixed(1),
      }));
      return {
        ...
        lines: rects.length,
        rects,
        elH: +el.getBoundingClientRect().height.toFixed(1),
      };
    }),
  );
  await ctx.close();
